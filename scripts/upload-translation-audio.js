/**
 * Translation Audio Upload Script
 * - Fetches metadata from YouTube API
 * - Downloads audio with yt-dlp
 * - Uploads to Appwrite (Translation Collection)
...
const COLLECTION_ID = process.env.APPWRITE_TRANSLATION_COLLECTION_ID;
const BUCKET_ID = process.env.APPWRITE_TRANSLATION_BUCKET_ID;
const YOUTUBE_PLAYLIST_URL = process.env.YOUTUBE_TRANSLATION_PLAYLIST_URL;
...
const TEMP_DIR = path.join(__dirname, "temp_translation_audio");
...
async function getPlaylistVideos() {
  console.log("📥 Fetching Translation playlist videos...");
 ...
 */
async function main() {
  console.log("🌍 Translation Audio Upload Process\n");
  ...
  if (!YOUTUBE_PLAYLIST_URL) {
    console.error("❌ Missing YOUTUBE_TRANSLATION_PLAYLIST_URL in .env.local");
    process.exit(1);
  }

  if (!COLLECTION_ID) {
    console.error(
      "❌ Missing APPWRITE_TRANSLATION_COLLECTION_ID in .env.local",
    );
    process.exit(1);
  }

  if (!BUCKET_ID) {
    console.error("❌ Missing APPWRITE_TRANSLATION_BUCKET_ID in .env.local");
    process.exit(1);
  }
