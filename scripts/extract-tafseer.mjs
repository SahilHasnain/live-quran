...
function main() {
  ...
  // Generate barrel files for tafseer and all translations
  const languages = ["en", "si", "ps", "sr"];

  // Tafseer barrel file
  const tafseerImportLines = [];
  const tafseerMapEntries = [];
  for (let i = 1; i <= 114; i++) {
    tafseerImportLines.push(`import t${i} from "./tafseer/${i}.json";`);
    tafseerMapEntries.push(`  ${i}: t${i} as TafseerData`);
  }

  const tafseerBarrel = `// Auto-generated by scripts/extract-tafseer.mjs
// Do not edit manually. Run \`npm run extract-tafseer\` to regenerate.

${tafseerImportLines.join("\n")}
...
${tafseerMapEntries.join(",\n")}
};

export default bundledTafseer;
`;

  writeFileSync(resolve(resolve(__dirname, "..", "data"), "bundled-tafseer.ts"), tafseerBarrel, "utf-8");
  console.log("Generated data/bundled-tafseer.ts barrel file");

  // Translation barrel files for each language
  for (const lang of languages) {
    const langImportLines = [];
    const langMapEntries = [];
    for (let i = 1; i <= 114; i++) {
      langImportLines.push(`import t${i} from "./translations/${lang}/${i}.json";`);
      langMapEntries.push(`  ${i}: t${i} as TranslationData`);
    }

    const langBarrel = `// Auto-generated by scripts/extract-tafseer.mjs
// Do not edit manually. Run \`npm run extract-tafseer\` to regenerate.

${langImportLines.join("\n")}
...
const bundledTranslation${lang.toUpperCase()}: Record<number, TranslationData> = {
${langMapEntries.join(",\n")}
};

export default bundledTranslation${lang.toUpperCase()};
`;

    const barrelPath = resolve(resolve(__dirname, "..", "data"), `bundled-translation-${lang}.ts`);
    writeFileSync(barrelPath, langBarrel, "utf-8");
    console.log(`Generated data/bundled-translation-${lang}.ts barrel file`);
  }
