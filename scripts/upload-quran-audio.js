...
const COLLECTION_ID = process.env.APPWRITE_TAFSEER_COLLECTION_ID;
const BUCKET_ID = process.env.APPWRITE_TAFSEER_BUCKET_ID;
const YOUTUBE_PLAYLIST_URL = process.env.YOUTUBE_TAFSEER_PLAYLIST_URL;
 ...
 */
async function main() {
  console.log("🎵 Quran Audio Upload Process\n");
  ...
  if (!YOUTUBE_PLAYLIST_URL) {
    console.error("❌ Missing YOUTUBE_PLAYLIST_URL in .env.local");
    process.exit(1);
  }

  if (!BUCKET_ID) {
    console.error("❌ Missing APPWRITE_TAFSEER_BUCKET_ID in .env.local");
    process.exit(1);
  }
