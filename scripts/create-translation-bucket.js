/**
 * Create Translation Storage Bucket in Appwrite
 * - Creates a new storage bucket for Translation audio files
...
/**
 * Create Translation storage bucket
 */
async function createTranslationBucket() {
  console.log("🗄️  Creating Translation Storage Bucket in Appwrite\n");
...
function updateEnvFile(bucketId) {
  ...
  if (
    ...
    envContent = envContent.replace(
      "APPWRITE_TRANSLATION_BUCKET_ID=YOUR_TRANSLATION_BUCKET_ID",
      `APPWRITE_TRANSLATION_BUCKET_ID=${bucketId}`,
    );
  } else if (envContent.includes("APPWRITE_TRANSLATION_BUCKET_ID=")) {
    // Update existing value
    envContent = envContent.replace(
      /APPWRITE_TRANSLATION_BUCKET_ID=.*/,
      `APPWRITE_TRANSLATION_BUCKET_ID=${bucketId}`,
    );
  } else {
...
async function main() {
  ...
  try {
    // Create bucket
    const bucketId = await createTranslationBucket();
    ...
    console.log("=".repeat(60));
    console.log("\nYou can now run: npm run upload-translation");
    console.log("\n");
  } catch (error) {
