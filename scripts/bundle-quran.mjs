...
async function main() {
  ...
  const mapEntries = [];
  for (let i = 1; i <= 30; i++) {
    importLines.push(`import p${i} from "./paras/${i}.json";`);
    mapEntries.push(`  ${i}: p${i} as BundledVerse[]`);
  }

  const barrelContent = `// Auto-generated by scripts/bundle-quran.mjs
...
`;
  ...
  writeFileSync(resolve(LOCAL_DATA, "bundled-paras.ts"), barrelContent, "utf-8");
  ...
  console.log(`Done! Generated data/paras/*.json, data/bundled-paras.ts, data/juzs.json`);
}
