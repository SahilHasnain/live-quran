/**
 * Create Translation Collection in Appwrite
 * - Creates a new collection with the same schema as Tafseer/Tilawat
...
/**
 * Create Translation collection with same schema as Tafseer/Tilawat
 */
async function createTranslationCollection() {
  console.log("🌍 Creating Translation Collection in Appwrite\n");
  ...
  try {
    ...
    // Create attributes (same schema as Tafseer/Tilawat)
...
function updateEnvFile(collectionId) {
  ...
  if (
    ...
    envContent = envContent.replace(
      "APPWRITE_TRANSLATION_COLLECTION_ID=YOUR_TRANSLATION_COLLECTION_ID",
      `APPWRITE_TRANSLATION_COLLECTION_ID=${collectionId}`,
    );
  } else if (envContent.includes("APPWRITE_TRANSLATION_COLLECTION_ID=")) {
    // Update existing value
    envContent = envContent.replace(
      /APPWRITE_TRANSLATION_COLLECTION_ID=.*/,
      `APPWRITE_TRANSLATION_COLLECTION_ID=${collectionId}`,
    );
  } else {
...
async function main() {
  ...
  try {
    // Create collection
    const collectionId = await createTranslationCollection();
    ...
    console.log("=".repeat(60));
    console.log("\nNext steps:");
    console.log("1. Set up Icecast mount point at /translation");
    console.log("2. Deploy Node.js streamer for Translation");
    console.log("3. Run: npm run upload-translation");
    console.log("\n");
  } catch (error) {
